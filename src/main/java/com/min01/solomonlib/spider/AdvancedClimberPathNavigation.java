...
public class AdvancedClimberPathNavigation<T extends Mob & IClimberEntity> extends AdvancedGroundPathNavigation<T> 
{
    ...
    public AdvancedClimberPathNavigation(T entity, Level worldIn, boolean checkObstructions, boolean canPathWalls, boolean canPathCeiling)
    {
    ...
    public void tick() 
    {
        ...
        if(!this.isDone())
        {
            ...
            if(!this.isDone()) 
            {
                ...
                if(moveController instanceof ClimberMoveControl && targetPoint instanceof DirectionalPathPoint && ((DirectionalPathPoint) targetPoint).getPathSide() != null)
                {
                    ((ClimberMoveControl<?>) moveController).setMoveTo(targetPos.x, targetPos.y, targetPos.z, targetPoint.asBlockPos().relative(dir), ((DirectionalPathPoint) targetPoint).getPathSide(), this.speedModifier);
                } 
                else 
                {
        ...
        else 
        {
            for(int obx = bx; obx < bx + sizeX2; ++obx) 
            {
                for(int obz = bz; obz < bz + sizeZ2; ++obz) 
                {
                    ...
                    {
                        BlockPathTypes nodeTypeBelow = this.nodeEvaluator.getBlockPathType(this.level, unswizzle(obx, by + (invertY ? 1 : -1), obz, ax, ay, az, Direction.Axis.X), unswizzle(obx, by + (invertY ? 1 : -1), obz, ax, ay, az, Direction.Axis.Y), unswizzle(obx, by + (invertY ? 1 : -1), obz, ax, ay, az, Direction.Axis.Z), this.mob);
                        ...
                        BlockPathTypes nodeType = this.nodeEvaluator.getBlockPathType(this.level, unswizzle(obx, by, obz, ax, ay, az, Direction.Axis.X), unswizzle(obx, by, obz, ax, ay, az, Direction.Axis.Y), unswizzle(obx, by, obz, ax, ay, az, Direction.Axis.Z), this.mob);
